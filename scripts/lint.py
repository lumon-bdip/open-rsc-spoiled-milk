...
def parse_javac(product: str, output: str) -> tuple[set[str], set[str]]:
    ...
    for line in output.splitlines():
        normalized_line = re.sub(r"^\[javac]\s+", "", line.strip())
        ...
        fingerprint = f"{product}|{path}|{category}|{message}"
        all_findings.add(fingerprint)
        if category in JAVAC_GATED_CATEGORIES:
            gated.add(fingerprint)
    return all_findings, gated
    ...
    if update:
        update_baseline(
            ...
            [
                ...
                "Stable format: product|path|warning-category|message",
            ],
            gated,
        )
    ...
    if update:
        update_baseline(
            ...
            [
                ...
                "Stable format: product|bug-type|priority|class|member",
            ],
            findings,
        )
