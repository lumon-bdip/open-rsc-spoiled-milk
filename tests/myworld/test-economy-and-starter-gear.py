...
def main() -> None:
    ...
    require("MyWorldItemOverrides.apply(items);" in client_entity_handler, "Client should apply generated MyWorld item overrides")
    client_item_overrides = CLIENT_ITEM_OVERRIDES.read_text(encoding="utf-8")
    require(
        "Generated by tools/generators/generate-item-overrides.py" in client_item_overrides,
        "Client item override table should identify its canonical generator",
    )
    require(
        "item == null || item.id != itemId" in client_item_overrides,
        "Client item overrides should verify the indexed item id",
    )
    require(
