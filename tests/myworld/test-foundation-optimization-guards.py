...
def main() -> None:
    ...
    require_snippet(GAME_STATE_UPDATER, "final boolean sendLegacyStaticScenePackets = shouldSendLegacyStaticScenePackets(player, skipStaticSceneScan);", "legacy static-scene packet handoff gate")
    require_snippet(GAME_STATE_UPDATER, "recordUpdateGameObjects(() -> sceneryChanged[0] = updateGameObjects(", "scenery object update view reuse")
    require_snippet(GAME_STATE_UPDATER, "player, visibleSceneryObjects, sendLegacyStaticScenePackets));", "scenery object update legacy packet handoff")
    require_snippet(GAME_STATE_UPDATER, "recordUpdateWallObjects(() -> wallsChanged[0] = updateWallObjects(", "wall object update view reuse")
    require_snippet(GAME_STATE_UPDATER, "player, visibleWallObjects, sendLegacyStaticScenePackets));", "wall object update legacy packet handoff")
    ...
    require_snippet(SERVER_CONFIGURATION, "public boolean WANT_SYNC_VISIBILITY_TICK_CACHE;", "tick snapshot cache config field")
    ...
    require_snippet(SERVER_CONFIGURATION, '"openrsc.syncVisibilityTickCache"', "tick snapshot cache system property")
