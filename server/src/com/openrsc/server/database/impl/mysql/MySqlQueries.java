...
public class MySqlQueries {
	...
	private String getDatabaseSkillColumn(SkillDef skill) {
		String skillName = skill.getShortName().toLowerCase();
		if ("melee".equals(skillName)) {
			return "attack";
		}
		if ("enchanting".equals(skillName)) {
			return "runecraft";
		}
		return skillName;
	}
	...
	public MySqlQueries(final Server server) {
		...
		for (SkillDef skill : getServer().getConstants().getSkills().skills) {
			String column = getDatabaseSkillColumn(skill);
			updateExperience = updateExperience + "`" + column + "`=?, ";
			updateStats = updateStats + "`" + column + "`=?, ";
			updateMaxStats = updateMaxStats + "`" + column + "`=?, ";
			playerExpCapped = playerExpCapped + "`" + column + "`, ";
			playerExp = playerExp + "`" + column + "`, ";
			playerCurExp = playerCurExp + "`" + column + "`, ";
			playerMaxExp = playerMaxExp + "`" + column + "`, ";
		}
