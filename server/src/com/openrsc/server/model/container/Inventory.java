...
public class Inventory {
	...
	public Boolean add(Item itemToAdd, boolean sendInventory) {
		synchronized (list) {
			...
			if (player.getConfig().WANT_MYWORLD && !itemDef.isStackable() && !itemToAdd.getNoted() && itemToAdd.getAmount() > 1) {
				boolean addedAll = true;
				final int amountToAdd = itemToAdd.getAmount();
				for (int i = 0; i < amountToAdd; i++) {
					if (!add(new Item(itemToAdd.getCatalogId(), 1, false), sendInventory)) {
						addedAll = false;
					}
				}
				return addedAll;
	...
	public void dropOnDeath(final Mob mob) {
		...
		if (!player.getConfig().WANT_MYWORLD && player.getPrayers().isPrayerActivated(Prayers.PROTECT_ITEMS) && iterator.hasNext()) {
			if (iterator.next().getDef(player.getWorld()).isStackable()) {
				iterator.previous();
			}
		}
