...
public class Item implements Comparable<Item> {
	...
	public Item(int catalogId, int amount, boolean noted, long itemId) {
		...
		itemStatus.setDurability(EnchantingItemEffects.getInitialItemDurability(catalogId));
	this.itemId = itemId;
	}
	...
	public Item copy() {
		return copyWithItemId(itemId);
	}

	public Item copyWithItemId(long newItemId) {
		Item copy = new Item(getCatalogId(), getAmount(), getNoted(), newItemId);
		copy.getItemStatus().setWielded(isWielded());
		copy.getItemStatus().setDurability(getItemStatus().getDurability());
		return copy;
	}
	//--------------------------------------------------------------
