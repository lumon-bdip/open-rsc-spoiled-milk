		...
		if (builder != null && possiblyValid) {
			switch (payload.getOpcode()) {
				...
				case SEND_STATUS_PROGRESS_BAR:
					...
					if (pb.interfaceId == 4) {
						builder.writeShort(pb.itemId);
						builder.writeShort(pb.delay);
					} else if (pb.interfaceId > 0 && pb.interfaceId != 2 && pb.interfaceId != 5) {
						if (pb.interfaceId == 1) {
							builder.writeShort(pb.delay);
						}
				...
				case SEND_MOVEMENT_UPDATE:
					MovementUpdateStruct movement = (MovementUpdateStruct) payload;
				...
				case SEND_PRODUCTION_INTERFACE:
					ProductionInterfaceStruct production = (ProductionInterfaceStruct) payload;
					builder.writeByte((byte) production.interfaceId);
					builder.writeByte((byte) production.actionId);
					if (production.actionId == 0) {
						...
						builder.writeByte((byte) recipeCount);
						for (int i = 0; i < recipeCount; i++) {
							builder.writeShort(production.itemIds[i]);
							builder.writeShort(production.requiredLevels[i]);
							builder.writeShort(production.inputAmounts[i]);
							builder.writeShort(production.outputAmounts[i]);
							builder.writeByte((byte) production.flags[i]);
							int ingredientCount = production.ingredientItemIds != null
								&& production.ingredientItemIds[i] != null
								? production.ingredientItemIds[i].length : 0;
							builder.writeByte((byte) ingredientCount);
							for (int j = 0; j < ingredientCount; j++) {
								builder.writeShort(production.ingredientItemIds[i][j]);
								builder.writeShort(production.ingredientFallbackItemIds[i][j]);
