...
public class ActionSender {
	...
	private static final PayloadGenerator<OpcodeOut> PAYLOAD_140_GENERATOR = new Payload140Generator();
	private static final PayloadGenerator<OpcodeOut> PAYLOAD_177_GENERATOR = new Payload177Generator();
	private static final PayloadGenerator<OpcodeOut> PAYLOAD_196_GENERATOR = new Payload196Generator();
	private static final PayloadGenerator<OpcodeOut> PAYLOAD_198_GENERATOR = new Payload198Generator();
	private static final PayloadGenerator<OpcodeOut> PAYLOAD_199_GENERATOR = new Payload199Generator();
	private static final PayloadGenerator<OpcodeOut> PAYLOAD_201_GENERATOR = new Payload201Generator();
	private static final PayloadGenerator<OpcodeOut> PAYLOAD_202_GENERATOR = new Payload202Generator();
	private static final PayloadGenerator<OpcodeOut> PAYLOAD_203_GENERATOR = new Payload203Generator();
	private static final PayloadGenerator<OpcodeOut> PAYLOAD_235_GENERATOR = new Payload235Generator();
	private static final PayloadGenerator<OpcodeOut> PAYLOAD_CUSTOM_GENERATOR = new PayloadCustomGenerator();
	...
	public static void hideProductionInterface(Player player, int interfaceId) {
		ProductionInterfaceStruct struct = ProductionInterfaceStruct.hide(interfaceId);
		tryFinalizeAndSendPacket(OpcodeOut.SEND_PRODUCTION_INTERFACE, struct, player);
	}
	...
	public static void sendEnterSleep(Player player) {
		if (!player.getConfig().FEATURES_SLEEP) {
	...
	public static void sendEquipmentStats(Player player, int slot) {
		...
		struct.weaponPowerPoints = player.getMeleeDefense();
		struct.magicPoints = player.getCarriedItems().getEquipment().getDisplayedMeleeOffense();
	...
	private static void debugJewelryEquipmentStats(final Player player, final EquipmentStatsStruct struct) {
	}

	private static String describeDebugItem(final Player player, final Item item) {
		if (item == null) {
			return "empty";
		}
		...
		return "id=" + item.getCatalogId()
			+ " name=\"" + item.getDef(player.getWorld()).getName() + "\""
			+ " slot=" + item.getDef(player.getWorld()).getWieldPosition()
			+ " wearable=" + item.getDef(player.getWorld()).getWearableId()
	...
	private static void sendQuestInfo(Player player) {
		...
		// Original clients expect exactly 50 quest-complete flags keyed by quest id.
		struct.questCompleted = new int[50];
		int i = 0;
		for (QuestInterface q : quests) {
			...
			if (q.getQuestId() >= 0 && q.getQuestId() < struct.questCompleted.length) {
				struct.questCompleted[q.getQuestId()] = player.getQuestStage(q) < 0 ? 1 : 0;
			}
			i++;
		}
	...
	public static void sendPrayerBook(Player player, PrayerCatalog.GodLine prayerBook) {
	...
	public static void sendStats(Player player) {
		...
		for (int i = 0; i < skillCount; i++) {
			int lvl = player.getSkills().getMaxStat(i);
			switch (player.getWorld().getServer().getConstants().getSkills().getSkillName(i)) {
				case "Melee":
				case "Attack":
	...
	public static void updateBankItem(Player player, int slot, Item newId, int amount) {
		 ...
		 */
		boolean sendWholeBank = (player.getClientVersion() == 10009 || player.getClientVersion() == 10010 || player.getClientVersion() == 10011 || player.getClientVersion() == 10012 || player.getClientVersion() == 10013);
	...
	public static void sendActionProgressBar(Player player, int itemId, int delayTicks) {
		ProgressBarStruct struct = new ProgressBarStruct();
		struct.interfaceId = 4;
		struct.itemId = itemId;
		struct.delay = Math.max(1, delayTicks) * player.getConfig().GAME_TICK;
		tryFinalizeAndSendPacket(OpcodeOut.SEND_STATUS_PROGRESS_BAR, struct, player);
	}

	public static void sendRemoveActionProgressBar(Player player) {
		ProgressBarStruct struct = new ProgressBarStruct();
		struct.interfaceId = 5;
		tryFinalizeAndSendPacket(OpcodeOut.SEND_STATUS_PROGRESS_BAR, struct, player);
	}
	...
	static void sendLogin(Player player) {
		try {
			if (player.getWorld().registerPlayer(player)) {
                ...
                sendPrayerBook(player, player.getPrayerBook());
