...
public class InterfaceOptionHandler implements PayloadProcessor<OptionsStruct, OpcodeIn> {
	...
	public void process(OptionsStruct payload, Player player) throws Exception {
		...
		if (option == null) {
			LOGGER.warn("Ignoring unknown interface option index={} player={} id={} amount={}",
				payload.index, player.getUsername(), payload.id, payload.amount);
			return;
		}
		...
		switch (option) {
			...
			case POINTS:
				...
				break;
			case PRODUCTION_SELECT_RECIPE:
			case PRODUCTION_QUANTITY_DOWN_ONE:
			case PRODUCTION_QUANTITY_DOWN_FIVE:
			case PRODUCTION_QUANTITY_UP_ONE:
			case PRODUCTION_QUANTITY_UP_FIVE:
				handleLegacyProductionOption(player, option);
				break;
			case PRODUCTION_START:
				handleProductionStart(player, payload);
				break;
	...
	private void handleAutoCastSpell(Player player, OptionsStruct payload) {
		...
		Spells spellEnum = Constants.spellToEnum(payload.id);
		if (!SpellHandler.isAutoCastableSpell(player, spellEnum, true)) {
			player.resetMagicCombat();
			player.setAutoCastSpell(null);
			return;
		}
		player.setAutoCastSpell(spellEnum);
	}

	private void handleProductionStart(Player player, OptionsStruct payload) {
		ProductionSession session = player.getAttribute("production_session", null);
		LOGGER.info("PRODUCTION_START player={} itemId={} amount={} hasSession={} hasStarter={}",
			player.getUsername(),
			payload.id,
			payload.amount,
			session != null,
			player.getAttribute("production_starter", null) != null);
		if (session == null) {
		...
		if (session.getRecipeByItemId(payload.id) == null) {
