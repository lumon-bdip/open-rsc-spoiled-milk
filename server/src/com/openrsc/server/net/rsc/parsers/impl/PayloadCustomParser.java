	...
	private boolean isPossiblyValid(Packet packet, Player player) {
		...
		switch (opcode) {
			...
			case ITEM_DROP:
				...
				if (player.getWorld().getServer().getConfig().WANT_MYWORLD) {
					return packet.getLength() >= 6;
				}
				return packet.getLength() >= 4;
			case OBJECT_COMMAND:
		...
		switch (opcode) {
			...
			case ITEM_DROP:
				...
				if (opcode == OpcodeIn.ITEM_COMMAND) {
					...
					if (player.getWorld().getServer().getConfig().WANT_DROP_X
						|| player.getWorld().getServer().getConfig().WANT_MYWORLD) {
						ic.amount = packet.readInt();
					}
					if (ic.index == -1 && packet.getReadableBytes() >= 2) {
						ic.realIndex = packet.readShort();
			...
			case INTERFACE_OPTIONS:
				...
				if (option == null) {
					LOGGER.warn("INTERFACE_OPTIONS unknown option index={} player={} packetId={}",
						os.index,
				...
				switch (option) {
					...
					case PRODUCTION_START:
						os.id = packet.readInt();
						os.amount = packet.readInt();
						LOGGER.info("PRODUCTION_PACKET_START player={} itemId={} amount={}",
							player != null ? player.getUsername() : "null", os.id, os.amount);
						break;
