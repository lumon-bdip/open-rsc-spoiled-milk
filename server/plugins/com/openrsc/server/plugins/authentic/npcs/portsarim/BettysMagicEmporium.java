...
public final class BettysMagicEmporium extends AbstractShop {
	...
	private void miceToMeetYou(final Player player, final Npc npc) {
		...
		if (ifvar("mice_to_meet_you", EAK_CAN_TALK)) {
			...
			if (option < 0 || option > 1) {
				...
				npcsay("I bought a bunch of supplies for my wizardshats and if I don't pay her soon");
	...
	private boolean isHoldingHat() {
		return ifheld(ItemId.WOOL_WIZARD_HAT.id(), 1);
	}

	private int isWearingHat(final Player player) {
		if (player.getCarriedItems().getEquipment().hasEquipped(ItemId.WOOL_WIZARD_HAT.id()))
			return ItemId.WOOL_WIZARD_HAT.id();
		return -1;
	}
}
