...
public class Bones implements OpInvTrigger, UseInvTrigger {
	...
	public boolean blockOpInv(Player player, Integer invIndex, Item item, String command) {
		if (item.getCatalogId() == ItemId.RASHILIYA_CORPSE.id()) return false;
		return command.equalsIgnoreCase("bury") || command.equalsIgnoreCase("scatter");
	}

	@Override
	public void onOpInv(Player player, Integer invIndex, Item item, String command) {
		if (item.getCatalogId() == ItemId.RASHILIYA_CORPSE.id()) return;
		if (command.equalsIgnoreCase("bury") || command.equalsIgnoreCase("scatter")) {

			if (item.getNoted()) {
				player.message(isAsh(item) ? "You can't scatter noted ash" : "You can't bury noted bones");
				return;
			}
	...
	private void buryBones(Player player, Item item) {
		...
		if (isAsh(item)) {
			player.playerServerMessage(MessageType.QUEST, "You scatter the ash");
		} else {
	...
	public boolean blockUseInv(Player player, Integer invIndex, Item item1, Item item2) {
		if (item1.getCatalogId() == ItemId.BONECRUSHER.id()) {
			return item2.getCatalogId() == ItemId.BONES.id()
				...
				|| item2.getCatalogId() == ItemId.DEMON_ASH.id();
			...
			return item1.getCatalogId() == ItemId.BONES.id()
				...
				|| item1.getCatalogId() == ItemId.DEMON_ASH.id();
		}
		return false;
	}

	private boolean isAsh(final Item item) {
		return item.getCatalogId() == ItemId.DEMON_ASH.id();
	}
}
