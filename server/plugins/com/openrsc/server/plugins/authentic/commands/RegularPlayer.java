...
public final class RegularPlayer implements CommandTrigger {
	...
	public void onCommand(Player player, String command, String[] args) {
		...
		if (command.equalsIgnoreCase("gang")) {
			...
			if (command.equalsIgnoreCase("pk") && !config().WANT_PVP) {
				player.message("This is a PvM-only world");
				return;
			}
	...
	private void queryCommands(Player player, int page) {
		if (page == 0) {
			String[] commands = getPageZeroCommands(player);
			if (player.getClientLimitations().supportsMessageBox) {
			...
			String[] commands = getPageOneCommands(player);
			if (player.getClientLimitations().supportsMessageBox) {
				ActionSender.sendBox(player, String.join("", commands), true);
			} else {
				for (String command : commands) {
	...
	private String[] getPageZeroCommands(Player player) {
		if (player.getConfig().WANT_PVP) {
			return pageZeroCommands;
		}

		return pageZeroCommandsNoPvp;
	}

	private String[] getPageOneCommands(Player player) {
		if (player.getConfig().WANT_PVP) {
			return pageOneCommands;
		}

		return pageOneCommandsNoDuel;
	}
	...
	private static final String[] pageZeroCommandsNoPvp = new String[]{
		"@yel@Commands available: %",
		"@lre@Type :: before you enter your command, see the list below. %",
		" %", // this adds a line of whitespace for readability
		"@whi@::gameinfo - shows player and server information %",
	...
	private static final String[] pageOneCommandsNoDuel = new String[]{
		"@yel@Commands available: %",
		"@lre@Type :: before you enter your command, see the list below. %",
		" %", // this adds a line of whitespace for readability
		"@whi@::time - shows the current server time %",
