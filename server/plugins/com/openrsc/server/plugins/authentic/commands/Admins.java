...
public final class Admins implements CommandTrigger {
	...
	private static final int ADMIN_RUNE_AMOUNT = 1000;
	private static final String DEATH_DROP_TEST_CACHE_KEY = "myworld_test_death_drops";
	private static final ItemId[] ADMIN_RUNE_KIT = new ItemId[] {
		ItemId.AIR_RUNE,
		ItemId.WATER_RUNE,
		ItemId.EARTH_RUNE,
		ItemId.FIRE_RUNE,
	...
	public void onCommand(final Player player, String command, String[] args) {
		...
		if (command.equalsIgnoreCase("saveall")) {
		...
		} else if (command.equalsIgnoreCase("hp1") || command.equalsIgnoreCase("onehp")) {
			setOwnHitsForTesting(player, 1);
		} else if (command.equalsIgnoreCase("poisonme") || command.equalsIgnoreCase("testpoison")) {
			poisonSelfForTesting(player, command, args);
		} else if (command.equalsIgnoreCase("deathdroptest") || command.equalsIgnoreCase("testdeathdrops")
			|| command.equalsIgnoreCase("toggledeathdrops")) {
			toggleDeathDropTesting(player, command, args);
			...
			|| command.equalsIgnoreCase("clearinventory") || command.equalsIgnoreCase("clearinv")) {
			removeItemInventoryAll(player, command, args);
		} else if (command.equalsIgnoreCase("wipebank") || command.equalsIgnoreCase("clearbank")) {
			if (args.length < 1) {
			...
			giveTools(player);
		} else if (command.equalsIgnoreCase("runes") || command.equalsIgnoreCase("allrunes") || command.equalsIgnoreCase("giverunes")) {
			giveRuneKit(player);
		} else if (command.equalsIgnoreCase("unlockspells") || command.equalsIgnoreCase("spellreqs") || command.equalsIgnoreCase("unlockspellreqs")) {
			unlockSpellRequirements(player, command, args);
	...
	private void giveRuneKit(Player player) {
		Inventory inventory = player.getCarriedItems().getInventory();
		int added = 0;
		synchronized (inventory) {
			for (ItemId rune : ADMIN_RUNE_KIT) {
				if (inventory.add(new Item(rune.id(), ADMIN_RUNE_AMOUNT))) {
					added++;
				}
			}
		}

		if (added == ADMIN_RUNE_KIT.length) {
			player.message(messagePrefix + "Added " + ADMIN_RUNE_AMOUNT + " of each rune to your inventory.");
		} else {
			player.message(messagePrefix + "Added " + added + " of " + ADMIN_RUNE_KIT.length + " rune stacks. Clear inventory space and try again.");
		}
	}

	private void unlockSpellRequirements(Player player, String command, String[] args) {
	...
	private void toggleNpcTrace(Player player, String command, String[] args) {
		int radius = player.getAttribute("debug_npc_trace_radius", 12);
		if (args.length >= 1) {
			try {
				radius = Math.max(1, Math.min(25, Integer.parseInt(args[0])));
			} catch (NumberFormatException e) {
				player.message(badSyntaxPrefix + command.toUpperCase() + " (radius 1-25)");
				return;
			}
		}

		boolean enabled = player.getAttribute("debug_npc_trace", false);
		if (enabled) {
			player.removeAttribute("debug_npc_trace");
			player.removeAttribute("debug_npc_trace_radius");
			player.removeAttribute("debug_npc_trace_last_custom_log");
			player.removeAttribute("debug_npc_trace_last_packet_log");
			player.removeAttribute("debug_npc_trace_budget");
			player.message(messagePrefix + "NPC trace disabled");
			return;
		}

		player.setAttribute("debug_npc_trace", true);
		player.setAttribute("debug_npc_trace_radius", radius);
		player.setAttribute("debug_npc_trace_last_custom_log", 0L);
		player.setAttribute("debug_npc_trace_last_packet_log", 0L);
		player.setAttribute("debug_npc_trace_budget", 200);
		player.message(messagePrefix + "NPC trace enabled with radius " + radius + " and 200 log samples. Check server log output.");
	}

	private void toggleWalkTrace(Player player) {
		boolean enabled = player.getAttribute("debug_walk_trace", false);
		if (enabled) {
			player.removeAttribute("debug_walk_trace");
			player.removeAttribute("debug_walk_trace_budget");
			player.message(messagePrefix + "Walk trace disabled");
			return;
		}

		player.setAttribute("debug_walk_trace", true);
		player.setAttribute("debug_walk_trace_budget", 0);
		player.message(messagePrefix + "Walk trace enabled. Check server log output.");
	}
	...
	private void setOwnHitsForTesting(final Player player, final int hits) {
		final int newHits = Math.max(1, Math.min(hits, player.getSkills().getMaxStat(Skill.HITS.id())));
		player.getUpdateFlags().setDamage(new Damage(player, player.getSkills().getLevel(Skill.HITS.id()) - newHits));
		player.getSkills().setLevel(Skill.HITS.id(), newHits);
		player.message(messagePrefix + "Set your current hits to " + newHits);
	}
	...
	private void poisonSelfForTesting(final Player player, final String command, final String[] args) {
		...
		if (args.length > 0) {
			try {
				poisonPower = Integer.parseInt(args[0]);
			} catch (NumberFormatException e) {
				player.message(badSyntaxPrefix + command.toUpperCase() + " (poison power)");
				return;
			}
		}
		if (poisonPower <= 0) {
			player.message(badSyntaxPrefix + command.toUpperCase() + " (poison power > 0)");
			return;
		}
		player.curePoison();
		player.applyPoison(poisonPower, poisonPower);
		player.message(messagePrefix + "Applied poison power " + poisonPower + " to yourself");
	}

	private void toggleDeathDropTesting(final Player player, final String command, final String[] args) {
		boolean enabled;
		if (args.length > 0) {
			...
			enabled = !player.getCache().hasKey(DEATH_DROP_TEST_CACHE_KEY);
		}

		if (enabled) {
			player.getCache().store(DEATH_DROP_TEST_CACHE_KEY, true);
			player.message(messagePrefix + "Death drop testing enabled: elevated item saving is disabled for you.");
		} else {
			player.getCache().remove(DEATH_DROP_TEST_CACHE_KEY);
			player.message(messagePrefix + "Death drop testing disabled: elevated item saving is restored.");
		}
	}
