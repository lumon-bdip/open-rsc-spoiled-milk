		...
		switch (obj.getID()) {
			case DOOR_ENTRANCE:
				if (obj.getLocation().equals(DOOR_LOCATION) && player.getX() <= 270) {
					if ((player.getCarriedItems().getEquipment().hasEquipped(ItemId.IRON_PLATE_MAIL_BODY.id()) || player.getCarriedItems().getEquipment().hasEquipped(ItemId.IRON_PLATE_MAIL_TOP.id()))
						&& player.getCarriedItems().getEquipment().hasEquipped(ItemId.LARGE_BRONZE_HELMET.id())) {
						doDoor(obj, player);
