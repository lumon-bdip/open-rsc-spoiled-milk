...
public class WitchesHouse implements QuestInterface, TalkNpcTrigger,
	...
	private void weakenPlayer(Player player) {
		...
		int[] stats = CombatEffectUtil.remapLegacyPlayerMeleeStats(player, Skill.ATTACK.id(), Skill.DEFENSE.id(), Skill.STRENGTH.id());
	...
	private boolean wearingInsulatingGloves(Player player) {
		return player.getCarriedItems().getEquipment().hasEquipped(ItemId.LEATHER_GLOVES.id())
			|| player.getCarriedItems().getEquipment().hasEquipped(ItemId.COW_HIDE_GLOVES.id())
			|| player.getCarriedItems().getEquipment().hasEquipped(ItemId.ICE_GLOVES.id());
	}

	//considerations: kite shields, large helms, plate bodies/tops, and plate legs
