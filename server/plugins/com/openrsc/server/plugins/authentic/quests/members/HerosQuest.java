...
public class HerosQuest implements QuestInterface, TalkNpcTrigger,
		...
		if (n.getID() == NpcId.GARV.id()) {
			...
			switch (player.getQuestStage(this)) {
				case 0:
					...
					choices.add(MyWorldQuestShortcuts.ALREADY_DONE_OPTION);
					...
					} else if (choices.get(opt).equals(MyWorldQuestShortcuts.ALREADY_DONE_OPTION)) {
					...
					} else if (canBuyCape(player) && choices.get(opt).equals("Is your cape also only for the foremost hero's of the land?")) {
						skillcape(player, n);
					}
					break;
				case 1:
	...
	private void skillcape(Player player, Npc n) {
		npcsay(player, n, "That cape is a relic of older training",
