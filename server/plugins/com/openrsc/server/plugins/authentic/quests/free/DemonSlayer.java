	...
	private void captainRovinDialogue(Player player, Npc n, int cID) {
		...
		if (cID == -1) {
			...
			if (choice == 0) {
				...
				if (choice != -1 && choices.get(choice).equalsIgnoreCase("Melee Skillcape")) {
					if (config().WANT_CUSTOM_SPRITES && getMaxLevel(player, Skill.ATTACK.id()) >= 99) {
						npcsay(player, n, "I see you too are a master of melee",
							"You are worthy to wield the Melee Skillcape",
		...
		switch (conversationID) {
			case GypsyConversation.INTRO:// Intro
				...
				if (introduceMenu == 0) {
					...
					npcsay(player, n, "Ok suit yourself");
				} else if (introduceMenu == 3) {
					MyWorldQuestShortcuts.completeDemonSlayer(player, n);
				}
				break;
