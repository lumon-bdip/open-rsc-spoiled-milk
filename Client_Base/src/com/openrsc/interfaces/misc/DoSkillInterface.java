...
public final class DoSkillInterface {
	...
	private ArrayList<ProductionRecipeView> productionRecipes;
	private String skillToDo, skillDoing;
	private String title = "";
	private boolean visible, rightClickMenu = false, productionMode = false;
	...
	public DoSkillInterface(mudclient mc) {
		...
		productionRecipes = new ArrayList<ProductionRecipeView>();
	}
	...
	private void renderProductionInterface() {
		...
		for (int i = 0; i < productionRecipes.size(); i++) {
			...
			ItemDef def = EntityHandler.getItemDef(recipe.getItemId());
			int column = i % columns;
			int row = i / columns;
			int boxX = startX + (column * itemBoxStepX);
			int boxY = startY + (row * itemBoxStepY);
			boolean hovered = mc.getMouseX() >= boxX && mc.getMouseX() <= boxX + itemBoxWidth
				&& mc.getMouseY() >= boxY && mc.getMouseY() <= boxY + itemBoxHeight;
			boolean selected = i == selectedRecipeIndex;
			int boxColour = recipe.isCraftable() ? 0x3A3026 : 0x222222;
			if (selected) {
				boxColour = 0x6E2E2E;
			} else if (hovered) {
				boxColour = recipe.isCraftable() ? 0x52463B : 0x2A2A2A;
			}
			...
			if (hovered && mc.mouseButtonClick == 1) {
			...
			if (hovered) {
				if (isFurnaceCategoryPicker()) {
					...
					hoverText = def.getName() + " - lvl " + recipe.getRequiredLevel()
						...
						+ outputUnitName(def, recipe.getOutputAmount());
					if (!recipe.isLevelMet()) {
						hoverText += " (level too low)";
					} else if (!recipe.isMaterialsMet()) {
						hoverText += " (missing materials)";
					}
				}
			}
		}
		...
		if (!hoverText.isEmpty()) {
			mc.getSurface().drawBoxAlpha(x + 12, y + 43, width - 24, 12, panelColour, 220);
	...
	private boolean isSmithingMaterialPicker() {
		return productionInterfaceId == PRODUCTION_SMITHING_MATERIAL;
	}

	private boolean isFurnaceCategoryPicker() {
		return productionInterfaceId == PRODUCTION_FURNACE_CATEGORY;
	}
	...
	private String outputUnitName(ItemDef def, int amount) {
		String name = def.getName().toLowerCase();
		if (name.endsWith(" bar")) {
			return amount == 1 ? "bar" : "bars";
		}
		return amount == 1 ? "item" : "items";
	}

	private String drawProductionIngredientCosts(ProductionRecipeView selected, int startX, int startY) {
		int count = selected.getIngredientCount();
		int stepX = Math.min(58, Math.max(42, (x + width - 110 - startX) / Math.max(1, count)));
		String hoverText = "";
		for (int i = 0; i < count; i++) {
			...
			drawString(amountText,
				...
				iconY + 36, 1, amountColour);
			boolean hovered = mc.getMouseX() >= iconX && mc.getMouseX() <= iconX + 32
				&& mc.getMouseY() >= iconY && mc.getMouseY() <= iconY + 38;
			if (hovered) {
				hoverText = ingredientTooltip(itemId, fallbackItemId, owned, required);
			}
		}
		return hoverText;
	}
	...
	private int getOwnedIngredientCount(int itemId, int fallbackItemId) {
		int owned = mc.getInventoryCount(itemId, false);
		if (isValidItemId(fallbackItemId)) {
			owned = Math.max(owned, mc.getInventoryCount(fallbackItemId, false));
		}
		return owned;
	}

	private boolean isValidItemId(int itemId) {
		return itemId >= 0 && itemId < EntityHandler.itemCount();
	}

	private void drawQuantityButton(int x, int y, int width, int height, String text, final int delta) {
		this.drawButton(x, y, width, height, text, 2, false, new ButtonHandler() {
			@Override
			void handle() {
	...
	private void sendProductionClose() {
		mc.packetHandler.getClientStream().newPacket(199);
		mc.packetHandler.getClientStream().bufferBits.putByte(20);
		mc.packetHandler.getClientStream().finishPacket();
	}
	...
	public void openProductionInterface(int interfaceId, String title, int inputItemId, int resourceAmount, int selectedRecipeId, int quantity,
		...
		if (this.selectedRecipeIndex < 0 && !this.productionRecipes.isEmpty()) {
			this.selectedRecipeIndex = 0;
		}
		setVisible(true);
	}

	public void closeProductionInterface() {
	...
	public void populateSkillItems() {
		...
		if (skillToDo.equals("Leather")) {
			...
			doSkillItems.add(new DoSkillItem(188, "8", ""));
		} else if (skillToDo.equals("Fletch oak")) {
			title = "Choose a bow to fletch";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(659, "19", ""));
			doSkillItems.add(new DoSkillItem(658, "22", ""));
		} else if (skillToDo.equals("Fletch willow")) {
			title = "Choose a bow to fletch";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(661, "26", ""));
			doSkillItems.add(new DoSkillItem(660, "29", ""));
		} else if (skillToDo.equals("Fletch maple")) {
			title = "Choose a bow to fletch";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(663, "40", ""));
			doSkillItems.add(new DoSkillItem(662, "43", ""));
		} else if (skillToDo.equals("Fletch yew")) {
			title = "Choose a bow to fletch";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(665, "47", ""));
			doSkillItems.add(new DoSkillItem(664, "50", ""));
		} else if (skillToDo.equals("Fletch magic")) {
			title = "Choose a bow to fletch";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(667, "61", ""));
			doSkillItems.add(new DoSkillItem(666, "64", ""));
			...
			title = "Choose an amount to string";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(649, "19", ""));
		} else if (skillToDo.equals("String oak long")) {
			title = "Choose an amount to string";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(648, "22", ""));
		} else if (skillToDo.equals("String willow short")) {
			title = "Choose an amount to string";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(651, "26", ""));
		} else if (skillToDo.equals("String willow long")) {
			title = "Choose an amount to string";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(650, "29", ""));
		} else if (skillToDo.equals("String maple short")) {
			title = "Choose an amount to string";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(653, "40", ""));
		} else if (skillToDo.equals("String maple long")) {
			title = "Choose an amount to string";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(652, "43", ""));
		} else if (skillToDo.equals("String yew short")) {
			title = "Choose an amount to string";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(655, "47", ""));
		} else if (skillToDo.equals("String yew long")) {
			title = "Choose an amount to string";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(654, "50", ""));
		} else if (skillToDo.equals("String magic short")) {
			title = "Choose an amount to string";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(657, "61", ""));
		} else if (skillToDo.equals("String magic long")) {
			title = "Choose an amount to string";
			skillDoing = "Fletching";
			doSkillItems.add(new DoSkillItem(656, "64", ""));
...
class ProductionRecipeView {
	...
	public int getIngredientCount() {
		return ingredientItemIds.length;
	}

	public int getIngredientItemId(int index) {
		return ingredientItemIds[index];
	}

	public int getIngredientFallbackItemId(int index) {
		return ingredientFallbackItemIds[index];
	}
	...
	public boolean isCraftable() {
		return isLevelMet() && isMaterialsMet();
	}
}
