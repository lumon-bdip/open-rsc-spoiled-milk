...
public class GraphicsController {
	...
	public Sprite spriteSelect(ItemDef item) {
		...
		if (location.length < 2
			|| !spriteTree.containsKey(location[0])
	...
	public final void drawSprite(Sprite sprite, int x, int y, int destWidth, int destHeight, int var5, int alpha) {
		try {
			try {
				int spriteWidth = sprite.getWidth();
				int spriteHeight = sprite.getHeight();
				...
				this.plot_tran_scale(heightStep, srcStartY, destWidth, (byte) -61, scaleY, spriteWidth, scaleX,
					destHeight, destHead, sprite.getPixels(), 0, srcStartX, destRowStride, alpha, this.pixelData);
			} catch (Exception var17) {
				System.out.println("drawSpriteAlpha: error in sprite clipping routine");
			}

		} catch (RuntimeException var18) {
			throw GenUtil.makeThrowable(var18,
				"ua.DA(" + x + ',' + y + ',' + destHeight + ',' + destWidth + ',' + alpha + ',' + sprite + ')');
		}
	}
