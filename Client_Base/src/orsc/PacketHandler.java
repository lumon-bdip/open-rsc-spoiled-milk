...
public class PacketHandler {
	...
	private SpriteDef getProjectileDefForUpdate(int sprite, String targetType, int targetServerIndex, int shooterServerIndex) {
		...
		return EntityHandler.projectiles.get(sprite);
	}
	...
	private static final Map<Integer, String> incomingOpcodeMap = new HashMap<Integer, String>() {{
		...
		put(136, "ONLINE_LIST");
		put(138, "PRODUCTION_INTERFACE");
		put(143, "SCENE_BASELINE");
	...
	private void handlePacket1(int opcode, int length) {
        ...
        /*else if(opcode == 50) { // Achievements
          ...
          return;
			...
			else if (opcode == 143) updateSceneBaseline(length);
	...
	public final void handlePacket2(int opcode, int length) {
		try {
			...
			// Show Bank
			...
			else if (opcode == 123) removeInventoryItem();
				...
				// Equipment Stats
			else if (opcode == 153) updateEquipmentStats(length);
	...
	private void updateBatchProgression() {
		...
		if (actionType == 4) {
			...
			int delayMillis = packetsIncoming.getShort();
			mc.startActionProgressBar(itemId, delayMillis);
			return;
		}
		if (actionType == 5) {
	...
	private void updateProductionInterface() {
		...
		if (actionId == 1) {
			mc.doSkillInterface.closeProductionInterface();
			return;
		}
		...
		int[] requiredLevels = new int[count];
		int[] inputAmounts = new int[count];
		int[] outputAmounts = new int[count];
		int[] flags = new int[count];
		int[][] ingredientItemIds = new int[count][];
		int[][] ingredientFallbackItemIds = new int[count][];
		int[][] ingredientAmounts = new int[count][];
		for (int i = 0; i < count; i++) {
			itemIds[i] = packetsIncoming.getShort();
			requiredLevels[i] = packetsIncoming.getShort();
			inputAmounts[i] = packetsIncoming.getShort();
			outputAmounts[i] = packetsIncoming.getShort();
			flags[i] = packetsIncoming.getByte() & 0xff;
			int ingredientCount = packetsIncoming.getByte() & 0xff;
			ingredientItemIds[i] = new int[ingredientCount];
			ingredientFallbackItemIds[i] = new int[ingredientCount];
			ingredientAmounts[i] = new int[ingredientCount];
			for (int j = 0; j < ingredientCount; j++) {
				ingredientItemIds[i][j] = packetsIncoming.getShort();
				ingredientFallbackItemIds[i][j] = packetsIncoming.getShort();
	...
	private void showOtherPlayers(int length) {
		...
		mc.traceLocalWalkServerUpdate(mc.getLocalPlayerX(), mc.getLocalPlayerZ(), currentX, currentZ, mc.getLocalPlayer());
		mc.setPlayerCount(0);
		if (needNextRegion && mc.consumeRegionLoadNeedsHardPlayerReset()) {
	...
	private void updateMovement() {
		...
		for (int i = 0; i < playerCount; i++) {
			...
			int z = packetsIncoming.getShort();
	...
	private void updateMovementSnapshot(int length) {
		...
		if (packetsIncoming.packetEnd + 16 <= length) {
			...
			if (packetsIncoming.packetEnd + 2 <= length) {
				npcCount = packetsIncoming.getShort();
				for (int i = 0; i < npcCount && packetsIncoming.packetEnd + 7 <= length; i++) {
					int serverIndex = packetsIncoming.getShort();
					int x = packetsIncoming.getShort();
					int z = packetsIncoming.getShort();
					int direction = packetsIncoming.getUnsignedByte();
	...
	private void updateSceneBaseline(int length) {
		int protocolVersion = 0;
		int serverTick = 0;
		int localX = 0;
		int localY = 0;
		...
		if (length >= 21) {
			...
			walls = packetsIncoming.getShort();
			groundItems = packetsIncoming.getShort();
			if (protocolVersion >= 5 && packetsIncoming.packetEnd + 2 <= length) {
				objectViewDistance = packetsIncoming.getShort();
			}
		}
		if (packetsIncoming.packetEnd + 20 <= length) {
			packetsIncoming.get32();
			packetsIncoming.get32();
			sceneryHash = packetsIncoming.get32();
	...
	private void applyCompleteSceneBaselineToLegacyLists(boolean force) {
		...
		for (int i = 0; i < mc.getWallObjectInstanceCount(); i++) {
			mc.dematerializeWallObjectInstance(i);
		}
	...
	private void addBaselineGameObject(SceneBaselineRecord record) {
		...
		try {
			mc.getWorld().registerObjectDir(localX, localZ, record.direction);
		} catch (RuntimeException ex) {
			System.out.println("Scene baseline loc error: " + ex.getMessage());
			return;
		}

		int instanceIndex = mc.getGameObjectInstanceCount();
		RSModel model = mc.getModelCacheItem(def.modelID).clone();
		applyExpandedGameObjectPickBounds(record.id, model);
		model.key = instanceIndex;
		model.addRotation(0, record.direction * 32, 0);
		model.setDiffuseLightAndColor(-50, -10, -50, 48, 48, true, 117);

		mc.setGameObjectInstanceX(instanceIndex, localX);
		mc.setGameObjectInstanceZ(instanceIndex, localZ);
		mc.setGameObjectInstanceID(instanceIndex, record.id);
		mc.setGameObjectInstanceDir(instanceIndex, record.direction);
		mc.setGameObjectInstanceModel(instanceIndex, model);
	...
	private void addBaselineWallObject(SceneBaselineRecord record) {
		...
		mc.setWallObjectInstanceX(instanceIndex, localX);
		mc.setWallObjectInstanceZ(instanceIndex, localZ);
		mc.setWallObjectInstanceID(instanceIndex, record.id);
		mc.setWallObjectInstanceDir(instanceIndex, record.direction);
	...
	private boolean usesExpandedGameObjectPickBounds(int objectId) {
		com.openrsc.client.entityhandling.defs.GameObjectDef def =
			com.openrsc.client.entityhandling.EntityHandler.getObjectDef(objectId);
		if (def == null) {
			return false;
		}
		if (objectId == 191) {
	...
	private void updateInventoryItem() {
		...
		logThrownAmmoAmount("inventory_slot", slot, itemID, noted, stackSize);
	...
	private void removeInventoryItem() {
		int slot = packetsIncoming.getUnsignedByte();
		int inventoryCount = mc.getInventoryItemCount();
		if (slot < 0 || slot >= inventoryCount) {
			return;
		}
		for (int i = slot; i < inventoryCount - 1; i++) {
			mc.setInventoryItem(i, mc.getInventoryItem(i + 1).clone());
		}
		Item lastItem = mc.getInventoryItem(inventoryCount - 1);
		lastItem.setItemDef((com.openrsc.client.entityhandling.defs.ItemDef) null);
		lastItem.setAmount(0);
		lastItem.setEquipped(false);
		lastItem.setNoted(false);
		lastItem.setCharges(0);
		lastItem.setDurability(0);
		mc.setInventoryItemCount(inventoryCount - 1);
	}
	...
	private void updateEquipmentSlot() {
		...
		if (id == 0xFFFF) {
			...
			logThrownAmmoAmount("equipment_slot", slot, id, false, amount);
		}
	...
	private void updateEquipment() {
		...
		for (int i = 0; i < equipCount; i++) {
			...
			logThrownAmmoAmount("equipment_full", equipslot, itemID, false, mc.equippedItemAmount[equipslot]);
		}
	}
	...
	private void updateNPCAppearances() {
		...
		for (int update = 0; numberOfUpdates > update; ++update) {
			...
			if (updateType == 1) { // NPC Chat
				...
				int amount = packetsIncoming.getUnsignedByte();
				int currentHits = packetsIncoming.getUnsignedByte();
				int maximumHits = packetsIncoming.getUnsignedByte();
				if (npc != null) {
					npc.healthMax = maximumHits;
						npc.healthCurrent = currentHits;
						npc.combatTimeout = Math.max(npc.combatTimeout, ORSCharacter.HIT_SPLAT_TIMEOUT);
						npc.setHitSplat(hitSplatType, amount);
					}
			} else if (updateType == 12) {
				boolean suppressAttackOption = packetsIncoming.getUnsignedByte() != 0;
				if (npc != null) {
					npc.suppressAttackOption = suppressAttackOption;
				}
				...
				if (npc != null) {
					...
					npc.direction = ORSCharacterDirection.lookup(direction);
				}
			} else if (updateType == 15) {
				boolean spiritSummon = packetsIncoming.getUnsignedByte() != 0;
				if (npc != null) {
					npc.spiritSummon = spiritSummon;
				}
			}
		}
	}
	...
	private void updateEquipmentStats(int length) {
		...
		for (int eq = 0; eq < intCount; ++eq) {
			int value = (int)(packetsIncoming.get32() & 0xffffffffL);
			if (eq < mc.playerStatEquipment.length) {
				mc.setPlayerStatEquipment(eq, value);
			}
		}
	}
		...
		for (int pp = 0; playerCount > pp; ++pp) {
			...
			if (updateType == 0) {
				...
				if (player != null) {
						player.healthMax = maxhp;
						player.healthCurrent = curhp;
						player.combatTimeout = Math.max(player.combatTimeout, ORSCharacter.HIT_SPLAT_TIMEOUT);
						player.setHitSplat(hitSplatType, amount);
						if (mc.getLocalPlayer() == player) {
							mc.setPlayerStatCurrent(3, curhp);
							mc.setPlayerStatBase(3, maxhp);
							mc.setShowDialogServerMessage(false);
